	...
	public void executeCommand(Command command) {
		...
		case JOIN:
			if (this.thread.getRoom() == ShortCircuitServer.getGetLobby()) {
				this.thread.getRoom().removeMember(this);
				this.thread
						.setRoom(ShortCircuitServer.getRoom(command.message));
				this.thread.getRoom().addMember(this);
				sendCommand(new Command(Command.CommandType.JOIN,
						command.message));
				this.thread.getRoom().broadcastCommand(
		...
		case DISCONNECT:
			...
			break;
		case ROOMS:
			sendCommand(new Command(Command.CommandType.ROOMS,
					this.thread.generateRoomList()));
			break;
		case USERS:
			sendCommand(new Command(Command.CommandType.USERS,
					this.thread.generateMembersList()));
			break;
		case START:
			break;
		case STOP:
